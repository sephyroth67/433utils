...
char RF_ROLLING_CODE1 = 0xDA ;
char RF_ROLLING_CODE2 = 0x98 ;
char RF_ROLLING_CODE3 = 0x1E ;
char RF_ROLLING_CODE4 = 0xE6 ;
char RF_ROLLING_CODE5 = 0x67 ;
...
void BlyssController::generate_rolling_code() {
  ...
  code = (rand() % 5) + 1; 
  if (code == 1) {
  this->buffer[4] = (this->buffer[4] & 0xF0) | ((RF_ROLLING_CODE1 & 0xF0) >> 4);
  this->buffer[5] = (this->buffer[5] & 0x0F) |(RF_ROLLING_CODE1 & 0x0F) << 4;
  }
  else if (code == 2) {
  this->buffer[4] = (this->buffer[4] & 0xF0) | ((RF_ROLLING_CODE2 & 0xF0) >> 4);
  this->buffer[5] = (this->buffer[5] & 0x0F) |(RF_ROLLING_CODE2 & 0x0F) << 4;
  }
  else if (code == 3) {
  this->buffer[4] = (this->buffer[4] & 0xF0) | ((RF_ROLLING_CODE3 & 0xF0) >> 4);
  this->buffer[5] = (this->buffer[5] & 0x0F) |(RF_ROLLING_CODE3 & 0x0F) << 4;
  }
  else if (code == 4) {
  this->buffer[4] = (this->buffer[4] & 0xF0) | ((RF_ROLLING_CODE4 & 0xF0) >> 4);
  this->buffer[5] = (this->buffer[5] & 0x0F) |(RF_ROLLING_CODE4 & 0x0F) << 4;
  }
  else if (code == 5) {
  this->buffer[4] = (this->buffer[4] & 0xF0) | ((RF_ROLLING_CODE5 & 0xF0) >> 4);
  this->buffer[5] = (this->buffer[5] & 0x0F) |(RF_ROLLING_CODE5 & 0x0F) << 4;
  }
