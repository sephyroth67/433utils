/*
  ...
  ---------------------------------
  1 : Default RCSwitch Protocol works with most power outlets
  2 : Not tested, documentation stated it works with Conrad RSL88R/B
  3 : Not tested, documentation stated it works with BL35P02
  5 : Rising Sun RSL887T1 (Low cost ceiling lamp switch named as venus) http://www.risingsun-elec.com/ProductOne.asp?ID=160
  4 : Idleman implementation of Home Easy for chacon devices
...
void RCSwitch::setProtocol(int nProtocol) {
  ...
  //protocol 6 is for Brennenstuhl RC 3600
  else if (nProtocol == 6) {
 ...
 * Returns a char[13], representing the Code Word to be send.
 *
 * getCodeWordE(char*, char*)
 *
 */
char* RCSwitch::getCodeWordE(char* sGroup, char* sDevice, boolean bStatus) {
   int nReturnPos = 0;
...
void RCSwitch::send(char* sCodeWord) {
  for (int nRepeat=0; nRepeat<nRepeatTransmit; nRepeat++) {
    int i = 0;
    while (sCodeWord[i] != '\0') {
 	if(i == 0 && this->nProtocol == 5){ //NO MATTERS WHAT VENUS FIRST BIT IS A 1
        this->send1();
      }
      else
      {
...
void RCSwitch::send0() {
	...
	else if (this->nProtocol == 2 || this->nProtocol == 6) {
		this->transmit(1,2);
	}
